...
export const read = (question: string) => new Promise<string>(resolve => {
  const rl = readline.createInterface({
    input: process.stdin,
    output: process.stdout,
  })

  const formated = t('>>> ' + question + ': ').color('cyan').style('bold').build()
...
export const confirm = (question: string) => new Promise<boolean>(resolve => {
  const rl = readline.createInterface({
    input: process.stdin,
    output: process.stdout,
  })

  const formated = t('>>> ' + question + ' (y/n): ').color('cyan').style('bold').build()
  rl.question(formated, (answer) => {
    rl.close()
    resolve(answer === 'y')
  })
