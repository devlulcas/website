...
<article class="relative flex flex-col overflow-hidden w-full h-80 bg-white rounded-lg shadow-lg">
	<picture aria-label={project.name} role="img" class="w-full h-full object-cover">
		<img loading="lazy" on:error={onError} src={project.picture} alt={project.name} />
	</picture>
	...
	<section
		class="absolute bottom-0 w-full h-1/2 flex flex-col flex-grow p-6 bg-opacity-80 backdrop-blur-md bg-white dark:bg-black dark:bg-opacity-50 overflow-hidden"
	>
		...
		<p
			class="h-full my-2 text-gray-700 dark:text-gray-200 text-md font-medium leading-[2ch] max-h-[4ch] overflow-hidden"
		>
			{project.description}
		</p>
