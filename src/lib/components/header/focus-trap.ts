...
export function focusTrap(node: HTMLElement, options: FocusTrapOptions = {}) {
  ...
  const trap = (fromObserver: boolean) => {
    ...
    const focusableElementsList = Array.from(
      node.querySelectorAll<HTMLElement>(focusableElements),
    );
    ...
    lastFocusableElement =
      focusableElementsList[focusableElementsList.length - 1];
  ...
  const unTrap = () => {
    firstFocusableElement?.removeEventListener(
      "keydown",
      onFirstFocusableKeyDown,
    );
    lastFocusableElement?.removeEventListener(
      "keydown",
      onLastFocusableKeyDown,
    );
  };
  ...
  return {
    update(newArgs: FocusTrapOptions = {}) {
      ...
      newArgs.returnFocus ? unTrap() : trap(false);
    },
    destroy() {
      unTrap();
      observer.disconnect();
