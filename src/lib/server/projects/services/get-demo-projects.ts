...
export async function getDemoProjects(): Promise<Project[]> {
  ...
  try {
    ...
    const isGithubProjectsResponse = (response: unknown): response is GithubDemoProjectsResponse => {
    ...
    const projectImages = await Promise.all(repositories.map((edge) => getProjectImage(edge.repository)));
